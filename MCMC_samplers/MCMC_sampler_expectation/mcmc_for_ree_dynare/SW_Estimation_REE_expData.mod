...
# cwly=1-crk*cky;
          ...
          q = -r+pinf_exp-1*eps_b +0*(1/((1-lambda/gamma)/(sigma_c*(1+lambda/gamma))))*eps_b + (crk/(crk+(1-delta)))*rk(1) +  ((1-delta)/(crk+(1-delta)))*q(1) ;
	      c = (lambda/gamma)/(1+lambda/gamma)*c(-1) + (1/(1+lambda/gamma))*c(+1) +((sigma_c-1)*cwhlc/(sigma_c*(1+lambda/gamma)))*(lab-lab(+1)) - (1-lambda/gamma)/(sigma_c*(1+lambda/gamma))*(r-pinf_exp + 1*eps_b) +0*eps_b ;
	      y = ccy*c+ciy*inve+eps_g  +  1*crkky*zcap ;
	      y = phi_p*( alpha*k+(1-alpha)*lab +eps_a );
	      pinf =  (1/(1+beta_bar*gamma*iota_p)) * ( beta_bar*gamma*pinf_exp +iota_p*pinf(-1) 
	      ...
	      w =  (1/(1+beta_bar*gamma))*w(-1)
               ...
               +(beta_bar*gamma)/(1+beta_bar*gamma)*pinf_exp
...
//sigma_l,0.5,0.01,10,NORMAL_PDF,2,0.75;
...
stderr eta_pi_exp,0.09,0.01,3,INV_GAMMA_PDF,0.1,2;

end;

varobs dy dc dinve labobs cpi_quarterly dw robs inf_fore_1_4;


estimation(optim=('MaxIter',2000),
mode_file=SW_Estimation_REE_expData_mode,
datafile=expectations_dataset,
nograph,
mode_compute=7,
first_obs=71,
//nobs=141,
...
//estimation(optim=('MaxIter',500),mode_file=SW_Estimation_REE_mode,datafile=sl+ eta_pi_expobodyan_dataset,nograph,mode_compute=1,first_obs=71,presample=4,kalman_algo=1,lik_init=2,prefilter=0,mh_replic=00000,mh_nblocks=1,mh_jscale=0.3,mh_drop=0.5,irf=100);//long training sample
