...
model.numvar_temp=model.numVar-7;   
sysmat.sigma_temp=sysmat.sigma(1:end-1,1:end-1);
...
sigma_vec=reshape(sysmat.sigma_temp,[length(sysmat.sigma_temp)^2,1]);
beta=zeros(model.numvar_temp,model.numvar_temp,model.N_fixedPoint);
%beta(:,:,1)=model.beta_init_FP;
beta(:,:,1)=0.5*eye(model.numvar_temp);
dist=1;
% i=0;
% while dist>eps_conv
msgstr=[];
...
%     i=i+1;
    ...
    M=gamma1+gamma2*betaAux^2;
         case 't'    
    M=(eye(model.numvar_temp)-gamma2*betaAux)\gamma1;
    gamma3=(eye(model.numvar_temp)-gamma2*betaAux)\gamma3_tilde;
    otherwise
...
lyap_b = gamma3 * sysmat.sigma_temp * gamma3';
% lyap_b= gamma3;
% try
