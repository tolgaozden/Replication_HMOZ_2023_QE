...
load results/msv_kf_output.mat;
...
S_exp(tt,:)= (a_tmp + b1_tmp .* a_tmp) + (b1_tmp.^2 + b2_tmp) .* S_FL(tt-2,:)' + (b1_tmp .* b2_tmp) .* S_FL(tt-3,:)';
  ...
  S_exp(tt,:) = a_tmp+ b_tmp * S(tt-2,model.BL_indices)' + d_tmp * S(tt-1,model.shock_indices([1 2 3 4 5 6 7 ]))';              
...
%     'inf_exp_correlations.xlsx','sheet','msv','range','B3');
