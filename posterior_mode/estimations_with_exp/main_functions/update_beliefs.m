...
%    thetaOld=[ beta_tt(model.FL_indices,[6 7 8 10 11 12 13]) dd_tt(model.FL_indices,[1 2 4 5 6 7  ])];
% 
%    regressor=[S(i-1,[6 7 8 10 11 12 13])';S(i-1,model.shock_indices([1 2 4 5 6 7 ]))'];
...
% beta_tt(model.FL_indices,[6 7 8 10 11 12 13])=theta(1:7,:)';
% dd_tt(model.FL_indices,[1 2 4  5 6 7 ])=theta(8:13,:)';
...
for jj=model.FL_indices
    index=index+1;
     ...
     thetaOld(index,:)=[ beta_tt(model.FL_indices(index),[6 7 8 10 11 12 13]) dd_tt(model.FL_indices(index),[1 2 4 5 6 7  ])];
     regressor=[S(i-1,[6 7 8 10 11 12 13])';S(i-1,model.shock_indices([1 2 4 5 6 7 ]))'];
     regressand=S(i,model.FL_indices(index))'; 
     
  [theta(index,:) rr_tt largest_eig pr_flag] =msv_learning(regressand,regressor,thetaOld(index,:),rr_old,gain,...
    model.FL_indices,model.BL_indices,model.numEndo,model.ridge_correction);   
        ...
        case 'var(1)'
            ...
            largest_eig=0;
            if i>2
%               thetaOld=[alpha_tt(model.FL_indices) beta_tt(model.FL_indices,model.BL_indices) dd_tt(model.FL_indices,2)];
thetaOld=[alpha_tt(model.FL_indices) beta_tt(model.FL_indices,model.BL_indices)];
   regressor=[1;S(i-2,model.BL_indices)'];
 regressand=S(i,model.FL_indices)'; 
...
beta_tt(model.FL_indices,model.BL_indices)=theta(2:model.numBackward+1,:)';
        ...
        case 'msv'
            ...
            %% shock enter with period t timing 
  ...
  %rr_old=rr_tt;
   regressor=[1;S(i-1,model.BL_indices)';S(i-1,model.shock_indices)']; % ;S(i-1,model.shock_indices_lagged(1))'
 regressand=S(i,model.FL_indices)'; 
...
alpha_tt(model.FL_indices,1)=theta(1,:)';
beta_tt(model.FL_indices,model.BL_indices)=theta(2:model.numBackward+1,:)';
dd_tt(model.FL_indices,:)=theta(model.numBackward+2:model.numBackward+model.numShocks+1,:)';
% ee_tt(model.FL_indices,model.shock_indices_lagged(1))=theta(model.numBackward+model.numShocks+2:end,:)';
    ...
    %% shock enter with period t-1 timing 
  ...
  %rr_old=rr_tt;
   regressor=[1;S(i-2,model.BL_indices)';S(i-1,model.shock_indices([1 2 3  5 6 7]))'];
 regressand=S(i,model.FL_indices)'; 
...
alpha_tt(model.FL_indices,1)=theta(1,:)';
beta_tt(model.FL_indices,model.BL_indices)=theta(2:model.numBackward+1,:)';
dd_tt(model.FL_indices,[1 2 3 5 6 7])=theta(model.numBackward+2:model.numBackward+7,:)';
    end
% ee_tt(model.FL_indices,model.shock_indices_lagged(1))=zeros(model.numForward,1);
            end
...
%=========================================================================
  ...
  thetaOld=[beta_tt(model.FL_indices,model.BL_indices) dd_tt(model.FL_indices,:) ]; % ee_tt(model.FL_indices,model.shock_indices_lagged(1))
  %rr_old=rr_tt;
   regressor=[S(i-1,model.BL_indices)';S(i,model.shock_indices)']; % ;S(i-1,model.shock_indices_lagged(1))'
 regressand=S(i,model.FL_indices)'; 
 
[theta rr_tt largest_eig pr_flag] =msv_learning(regressand,regressor,thetaOld,rr_old,gain,...
    model.FL_indices,model.BL_indices,model.numEndo,model.ridge_correction);

beta_tt(model.FL_indices,model.BL_indices)=theta(1:model.numBackward,:)';
dd_tt(model.FL_indices,:)=theta(model.numBackward+1:model.numBackward+model.numShocks,:)';
...
% alpha_tt(jj)=0;
    ...
    [alpha_tt(jj),beta_tt(jj,jj),rr_tt(2,2,jj)]=...
    cgl_learning_recursive(S(i,jj),S(i-1,jj),alpha_tt(jj),beta_tt(jj,jj),rr_tt(2,2,jj),gain(index_gain));
largest_eig=0;
pr_flag=0;
                           end
            end
...
beta1_tt=zeros(model.numEndo,model.numEndo);
beta1_tt(model.FL_indices,model.FL_indices)=  diag(beta_tt(1:7));

beta2_tt=zeros(model.numEndo,model.numEndo);
beta2_tt(model.FL_indices,model.FL_indices)=  diag(beta_tt(8:14));
...
beta_tt = [diag(beta1_tt(model.FL_indices,model.FL_indices));diag(beta2_tt(model.FL_indices,model.FL_indices))];
                           end
