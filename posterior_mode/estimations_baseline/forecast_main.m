...
load('dynare_initial_beliefs/slobodyan_dataset.mat');
  ...
  S_fore_all= nan(forecast.numVar,forecast.horizon,forecast.num_periods);%first element 2*forecast.numVar only if ar(2) with t-1
...
model.PLM='var(1),t-1';%options: 'msv,t','msv,t-1',ar(1),t,rls','ar(1),t,sac','ar(1),t-1,rls','ar(1),t-1,sac','ar(2),t-1,rls'
model.learning=1; %options: 1=adaptive learning invoked, 0=beliefs fixed at initial values 
model.learning_algo='var(1)'; %options: 'msv','ar(1),rls','ar(1),sac','msv,restricted';
model.initial_beliefs='var(1),ree-based,fixed';%options: 'msv,ree-based,fixed','ar(1),ree-based,fixed','ar(1),ree-based','ar(1),ble-based','ar(1),ble-based,fixed'
...
file_name=['auxiliary_forecast_model_var1' num2str(jj) '.mat']
