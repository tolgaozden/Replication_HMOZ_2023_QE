...
% regressor=[const lagged shocks(:,2)];
 regressor=[const lagged];
regressand=contemp;
...
% dd_init=0*regression(9:15,:)';
dd_init = zeros(7,7);
% dd_init(:,2) = regression(9,:);

% ee_init=zeros(size(dd_init));
% ee_init=zeros(size(dd_init,2),1)';
% ee_init(1,shock_indices_lagged)=regression(16,:)';
save var1_initial_beliefs alpha_init beta_init dd_init  rr_init;%ee_init
